...
export default function DashboardHeader({ name, activeVouchers, membership = 'Gold Member', avatarUrl }: DashboardHeaderProps) {
        ...
        <header className="flex h-[66px] items-center justify-between">
            <div className="flex flex-col gap-1">
                ...
                <div className="flex items-center gap-2">
                    <span className="flex items-center gap-1 rounded-full bg-[#00776a] px-4 py-1 text-[13px] leading-4 font-semibold tracking-[0.6px] text-[#84ffeb]">
                        <Crown size={12} /> {membership}
                    </span>
                    <span className="text-[15px] leading-5 text-[#6e7977]">{activeVouchers} Active Vouchers</span>
                </div>
            </div>
