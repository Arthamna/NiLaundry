...
const BRANCHES = [
    'Jalan Teknik Kimia No. 99, Keputih, Sukolilo, Surabaya, Jawa Timur',
    'Cabang Darmo, Surabaya',
    'Cabang Gubeng, Surabaya',
];

const SERVICES: { name: string; pricePerKg: number }[] = [
    { name: 'Laundry', pricePerKg: 7000 },
    { name: 'Cuci Setrika', pricePerKg: 10000 },
    { name: 'Dry Clean', pricePerKg: 25000 },
    { name: 'Setrika Saja', pricePerKg: 5000 },
];

const priceOf = (service: string) => SERVICES.find((s) => s.name === service)?.pricePerKg ?? 0;
                ...
                <div className="flex items-center gap-[36px]">
                    <div className="flex items-center gap-[12px]">
                        ...
                        <div className="relative w-[400px]">
                            <select
                                value={item.service}
                                onChange={(e) => onChange({ ...item, service: e.target.value })}
                                aria-label="Service"
                    ...
                    <div className="flex h-[28px] w-[191px] items-center justify-between">
                        ...
                        <div className="flex h-full items-center gap-[4px]">
                            ...
                            />
                            <span className="text-[14px] leading-[16.5px] text-[#62748e]">kg</span>
                        </div>
                    </div>
                </div>
...
export default function NewOrderPage() {
    ...
    const [items, setItems] = useState<OrderItemState[]>([
        // { id: 0, service: 'Laundry', quantity: '', notes: '' },
        // { id: 1, service: 'Laundry', quantity: '', notes: '' },
    ]);
    const [submitting, setSubmitting] = useState(false);

    const subtotals = items.map((it) => priceOf(it.service) * (Number(it.quantity) || 0));
    ...
    function addItem() {
        setItems((prev) => [...prev, { id: nextId++, service: 'Laundry', quantity: '', notes: '' }]);
    }

    async function handleMakeOrder() {
        setSubmitting(true);
        await createOrder(USER_ID, {
            service: items.map((it) => `${it.service} (${it.quantity || 0}kg)`).join(', '),
            pickupAddress: branch,
            notes: items.map((it) => it.notes).filter(Boolean).join(' | '),
        });
        setSubmitting(false);
